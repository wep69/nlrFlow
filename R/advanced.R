...
#' }
#' @export
nl_saemix <- function(model_function,data,psi0,id,predictor,response,parameter_names,covariate_model=NULL,transform_par=rep(0,length(parameter_names)),...) {.nl_require("saemix","SAEM nonlinear mixed-effects models"); modpar_arg <- if ("name.modpar" %in% names(formals(saemix::saemixModel))) "name.modpar" else "parameter.names"; sm<-saemix::saemixModel(model=model_function,description="nlrFlow SAEM model",psi0=psi0,transform.par=transform_par,covariate.model=covariate_model); sm[[modpar_arg]] <- parameter_names; sd<-saemix::saemixData(name.data=data,name.group=id,name.predictors=predictor,name.response=response);fit<-saemix::saemix(sm,sd,...);.nl_wrap(fit,"saemix",stats::as.formula(paste(response,"~",predictor)),data,psi0,metadata=list(parameter_names=parameter_names,id=id))}
