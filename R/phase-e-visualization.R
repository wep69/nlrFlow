...
.nl_phase_e_plot <- function(object, x=NULL, y=NULL, ...) {
  ...
  if(inherits(object,"nlr_pinn")) {
    ...
    pred<-grep("_predicted$",names(d),value=TRUE)[1];obs<-sub("_predicted$","_observed",pred);return(ggplot2::ggplot(d,ggplot2::aes_string(x=names(d)[1],y=pred))+ggplot2::geom_line(linewidth=.8)+ggplot2::geom_point(ggplot2::aes_string(y=obs),alpha=.65)+ggplot2::theme_classic(base_size=11)+ggplot2::labs(y=object$response,subtitle=paste("PINN:",object$problem)))
  }
